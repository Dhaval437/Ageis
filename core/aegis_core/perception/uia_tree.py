...
What a walk cannot see: an Electron or Chromium app that has not turned its
accessibility on exposes its web content as one empty `Document`. VS Code opts in;
Notion, measured here, does not, and gives 15 elements, all window chrome, for a
full page. That is the app's choice, not a failed read, and `P2-07`'s OCR is the
fallback for it.
...
UIA_IS_VALUE_PATTERN_AVAILABLE: Final = 30043
...
PROPERTIES: Final = (
    ...
    UIA_IS_PASSWORD,
    UIA_IS_OFFSCREEN,
    UIA_IS_VALUE_PATTERN_AVAILABLE,
    UIA_VALUE_VALUE,
)
...
def _element(node: UiaNode, index: int, parent: int | None, depth: int) -> UiaElement:
    ...
    A cached property an element does not support reads as that property's
    *default* — `""` for a value — so whether the element has a value at all is
    asked separately, through `IsValuePatternAvailable`. Without that, a button
    and an empty text box would look the same.
    """
    is_password = node.prop(UIA_IS_PASSWORD) is True
    has_value = node.prop(UIA_IS_VALUE_PATTERN_AVAILABLE) is True
    return UiaElement(
        id=index,
        parent=parent,
        depth=depth,
        ...
        value=_text(node.prop(UIA_VALUE_VALUE)) if has_value and not is_password else None,
...
def _client_module() -> Any:
    ...
    Generating it writes Python files into `comtypes.gen`, so two threads walking
    for the first time at once must not both do it.
    """
    with _client_lock:
