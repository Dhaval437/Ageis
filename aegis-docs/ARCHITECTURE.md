...
│  │  ├─ actuation/               # input.py (SendInput), keys.py, motion.py, window.py, preempt.py, killswitch.py, win32.py
│  │  ├─ guardian/                # policy.py, rules.yaml, rules.py, risk.py, scope.py, allow_rules.py, approvals.py, win32.py
│  │  ├─ recovery/                # journal.py, undo.py, snapshot.py
│  │  ├─ storage/                 # db.py, migrations/, audit.py, vault.py, usage.py, settings.py, scopes.py, allow_rules.py
...
- **Stuck detector.** If the last 3 observations are near-identical (perceptual hash distance ≤ `phash.NEAR_IDENTICAL_BITS`, see § 6.3) and the plan has not changed, declare stuck, stop, and ask the user. Repeating a failing click is the single most common agent failure mode; treat it as a first-class error.
...
2. `screen.py` captures the monitor (or window) via `mss` and downscales. `mark.py` then draws the numbered boxes over candidate elements ("set-of-mark" prompting) that let the model return an **element id**, not raw coordinates: `mark(redacted, pruned)` outlines each candidate and labels it with its **walk `id`** — the same id `prune()` kept and `grounding.resolve()` takes — and returns the encoded `Screenshot` with the ids it actually drew. It takes a `Redacted`, never a `Frame`, so a mark can only ever sit on pixels redaction has already cleared to leave; it draws digits and nothing else; and the marks go on **after** the downscale, at the size the model sees. One walk at a time, since walk ids are unique within a walk and not across walks. A candidate off the captured region or too small to point at at the output scale comes back in `Marked.unmarked` instead of being offered as an id the model cannot see.
3. `ocr.py` (Windows OCR, `Windows.Media.Ocr` via the `winrt` projection, on its own daemon thread, 3 s timeout) fills the gap for windows that draw their own content: a canvas, a terminal buffer, an editor or page rendered with accessibility off. `blind_regions(tree)` finds the large parts of a window that the tree's describing elements — anywhere in the tree, since a Chromium page lives in another branch from its render pane — cover less than 25 % of; a large element's *name* is a title, and a large *value* counts as content only when nothing inside it speaks. **`redact()` is the only caller**: it reads each blind region off the frame *after* the tree-based painting, so OCR never sees a secret field or another window, and puts every line through the same credential and secret-label rules. A blind region is read and redacted, or painted black (`Redacted.unread`) — never sent unread. The lines come back as `Redacted.text`. A Tesseract fallback for machines with no OCR language is `P2-12`.
4. `grounding.py` turns `element_id → click point`: `resolve(pruned, element_id, offered=marked.ids)` refuses an id the model was not shown, then takes a **fresh walk** of the window and re-finds the element by UIA `runtime_id`, requiring the same role and (redacted) name, enabled, on screen, under the same `DisplayLayout`. The point is the centre of the element's **fresh** visible rectangle — already physical pixels, so there is no DPI step. Before it is returned, Windows' hit test (`WindowFromPoint`) must land in the element's window and UIA's hit test must land on the element, inside it, or on a bare ancestor (coarse providers); any *other* element there refuses. Every refusal is a `GroundingError` telling the agent to observe again. The `Grounding` carries its `layout`, which the click re-verifies immediately before it fires. Known false refusals, in the safe direction: title-bar min/max/close buttons (UIA answers them under a different id, or a pane from another subtree) — the `window` tools own those. **Text lines** (`P2-13`): a window that draws its own content has OCR text and no element ids, so `text_targets(redacted)` offers the lines of `Redacted.text` by index — never one redaction blanked, and only one that lies in exactly one walked window's blind region — and `resolve_text(redacted, line_id, offered=…)` grounds the one chosen. That is the coordinate path of last resort, so the proof is the pixels: the window is walked, captured and put through `redact()` again (OCR's only route to the screen), and the line must be read there with the **same text in the same place** (each edge within `TEXT_PLACE_PX` = 2), and `WindowFromPoint` must land in the window. A line that changed, scrolled, was covered, now shows a secret, or could not be read again refuses. It returns a `TextGrounding` with the same `point` / `box` / `hwnd` / `layout` a click needs. Not seen: an invisible element in the same window that takes clicks over the text without drawing.
5. `phash.py` reduces an observation to a 256-bit **difference hash** for the stuck detector: `phash(redacted)` shrinks the *redacted* frame to a 17x16 grey grid, and each bit says whether a cell is brighter than its right-hand neighbour by more than one level (the dead band keeps rounding ties between flat cells from flipping bits). Two hashes are compared by Hamming distance; `near_identical()` is `≤ NEAR_IDENTICAL_BITS` (5). Measured on a real 3200x2000 desktop: a static screen 0 bits, a tooltip 2, a dropdown 9, a dialog 33, ~11 ms per hash. A caret, a typed character or a checkbox moves nothing — the question is whether the screen *meaningfully* changed, and a stuck call also needs an unchanged plan. `PerceptualHash.hex` (64 lowercase hex digits) is what `observations.phash` stores.
...
allow_rules(id, kind, tool, folder, params_digest, task_id, created_at)   -- P3-11, m0004
```
...
*Approvals and always-allow rules (P3-11).* A `confirm` goes through `guardian/approvals.py`'s `ApprovalBroker.request()`, which publishes `approval.requested` and waits for `POST /v1/approvals/{id}`. The deadline lives **in the core**, not the dialog: 30 s for an effective tier of `DANGEROUS`, 60 s otherwise, and it ends in `deny`; the kill switch, a cancelled wait and shutdown deny everything pending, and a second or late answer is a 404, so a click can never approve something that has already been decided. *Allow always* makes a rule from the **pending call** — tool, params digest, the folder its own paths share, or its task — and the request names only the *kind*, so the renderer never supplies a path. Rules live in the `allow_rules` table (every kind `CHECK`ed to carry exactly what it matches on) and are listed and revoked through `GET`/`DELETE /v1/rules`. In `evaluate()` they are step 4, after the signals: under `trusted` only (`§ 10`), for an effective tier of `SAFE` or `CAUTION` only, never over an `echoes_observation` confirm and never over any `deny` — so what one actually relaxes is a read outside the scope, and a folder rule matches only when *every* target is a path inside that folder (a command or registry key whose text looks like one never matches).

*The list (P3-09).* `rules.yaml` holds 28 entries and 100 patterns: writes into `%WINDIR%`, Program Files, the Startup folders and Aegis' own install directory; **any** access to Windows' credential stores and DPAPI keys, browser password/cookie/card databases and their `Local State` key (Chromium, Opera, Firefox, legacy IE), `.ssh`, `.gnupg`, cloud CLIs' credentials, token files (`.git-credentials`, `.netrc`, `.pypirc`), password-manager vaults and extension storage, crypto wallets, and all of `%AEGIS_DATA%` (the DB with the audit log and journal, logs, screenshots, the kill-switch binding); registry writes to autostart keys (Run*, Winlogon, IFEO), Defender/firewall/UAC/LSA policy and services/drivers, and any access to `HKLM\SAM`/`SECURITY`; and commands that wipe disks, change boot configuration, delete shadow copies or backups, tamper with Defender, the firewall or UAC, install services or drivers, clear event logs, read credential stores or dump LSASS, write autostart keys, or run base64-**encoded PowerShell** (a command the Guardian cannot read). `%AEGIS_DATA%` and `%AEGIS_INSTALL%` are **computed by the core** — from `storage.db.default_data_dir()` and from where the process runs (`<install>\resources\core\aegis-core.exe` packaged, the repository in development) — so no environment variable can move them. Command rules are searched in the text as written **and** with `^`, backticks and quotes removed (`for^mat`, `"vss"admin`). This is defence in depth on text, not a parser: a path inside a command is not resolved, and `run_powershell` is DANGEROUS regardless. `tests/guardian/test_forbidden.py` holds a denied example for every entry and **every pattern** (a pattern no example reaches fails the build), and a list of ordinary work beside each place — reading `notepad.exe`, a browser's history, `vssadmin list`, `Get-Date -Format` — that must stay allowed.
...
- **Kill switch:** global hotkey (default `Ctrl+Alt+Shift+Q`, rebindable, registered in Electron MAIN so it works even if the core hangs) → immediate hard stop: cancel all tasks, release keys, kill child job processes, freeze the agent, surface the last 5 actions with an Undo offer. Also available as a persistent overlay button and a tray item.
  ...
  - *Held keys (P3-15).* MAIN releases Shift, Ctrl, Alt and Windows, both sides, through its own keyboard (`main/win-input.ts`: `GetAsyncKeyState` + `SendInput` via `koffi`, scan codes, extended flag and `AEGIS_SIGNATURE`, as the core sends them): straight after `supervisor.terminate()` kills a core — the kill switch's last resort and the watchdog — and again when that process's exit arrives; first thing when a core dies on its own while a task was running; and after a kill a core acknowledged with `release_failures > 0`. Measured: a held Shift and right Ctrl come up in well under 20 ms, inside § 13's 200 ms. Job children are stopped by the **Job Object** `P5-08` must put them in (`JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE`), so they die with the core; MAIN does not walk the process tree, because `taskkill /T` took ~300 ms here and a tree walk by parent PID loses a child whose parent has already died. The "Stopped by you" UI and the last-five-actions view are `P3-14`.
...
- **Text with no tree behind it** — a canvas, a terminal buffer, a page with accessibility off — is read by OCR inside `redact()` and put through the same scanner: a line with a credential or a secret label is blacked out in the pixels and replaced in the text. A region OCR cannot read is painted black (§ 6.3).
...
- **`approval.requested`** (P3-11) carries `{approval_id, tool, prompt, why, tier, reason, timeout_s, allow_always, rule_kinds, folder, reversible}` (`reversible` is true only when the caller says the tool has a real `undo()`): `prompt` is the tool's `describe()` sentence and `why` the agent's reason, each clipped to 4 000 characters; `allow_always` is false wherever a rule could never apply (a `DANGEROUS` effective tier, an echo of screen text, no rule store); `rule_kinds` lists only the scopes this call supports, and `folder` is the one a `tool_in_folder` rule would cover. **`approval.resolved`** carries `{approval_id, choice, decided_by: user|timeout|stopped, rule_id}` and is published for every approval exactly once — answered, timed out, stopped by the kill switch, or cancelled with its task — so a dialog is always dismissed.
