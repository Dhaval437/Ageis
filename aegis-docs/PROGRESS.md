...
| **Current task** | P0-09 — Renderer event-stream client + store (next) |
| **Last session** | 2026-09-13 — P0-08 WebSocket event hub |
| **Overall** | 10 / 103 tasks |
...
|---|---|---|---|---|---|
| P0 | Foundations & plumbing | 17 | 8 | 🟨 | App launches, core handshake works, one round-trip |
...
| P0-09 | Renderer event-stream client + Zustand store; UI is a pure function of the stream | P0-08, P0-03 | TODO | Includes MAIN's side: `core.subscribe` has no WS client behind it yet. MAIN must hold the connection (token stays in MAIN), reconnect with `since`, handle every close code in `ARCHITECTURE.md § 9.2`, and **drop the cursor whenever the supervisor starts a new core** — `seq` restarts at 1 until P6-06 |
