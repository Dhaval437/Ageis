...
| **Current task** | P0-07 — Supervisor (next) |
| **Last session** | 2026-09-09 — P0-06 startup handshake |
| **Overall** | 8 / 101 tasks |
...
|---|---|---|---|---|---|
| P0 | Foundations & plumbing | 16 | 6 | 🟨 | App launches, core handshake works, one round-trip |
...
| P0-07 | Supervisor: spawn, health-check, 3-strike respawn, kill core when MAIN exits | P0-06 | TODO | Test: kill MAIN → core gone in ≤2 s. Starts from `main/handshake.ts` + `main/core-gateway.ts`; also owns wiring `rest.core` in `index.ts` and resolving the core's path (dev interpreter vs packaged `aegis-core.exe`) |
