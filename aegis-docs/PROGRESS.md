...
| **Current task** | P0-11 — Pydantic→TS type generation (next) |
| **Last session** | 2026-09-14 — P0-10 SQLite bootstrap + migrations |
| **Overall** | 12 / 104 tasks |
...
|---|---|---|---|---|---|
| P0 | Foundations & plumbing | 17 | 10 | 🟨 | App launches, core handshake works, one round-trip |
...
| P0-11 | Pydantic→TS type generation script wired into `pnpm dev` | P0-05, P0-03 | TODO | CI fails if generated types are stale. **Must replace the hand-seeded `EVENT_TYPES`/`EventType`/`StreamEvent` at the bottom of `packages/shared/src/api.ts`** (P0-09) with generated output of the same names and shapes |
